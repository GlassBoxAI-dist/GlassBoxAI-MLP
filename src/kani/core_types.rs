...
pub const MAX_FFI_STRING_LEN: usize = 4096;
pub const MAX_FFI_ARRAY_LEN: usize = MAX_ARRAY_SIZE;

pub fn validate_i32_as_usize(val: i32) -> Option<usize> {
    if val < 0 {
        None
    } else {
        Some(val as usize)
...
pub fn validate_i32_positive(val: i32) -> Option<usize> {
    if val <= 0 {
        None
    } else {
        Some(val as usize)
...
pub fn validate_ffi_len(len: i32, max: usize) -> Option<usize> {
    if len < 0 {
        return None;
    }
...
pub fn validate_f64_param(value: f64) -> Option<f64> {
    if value.is_nan() || value.is_infinite() {
        None
    } else {
        Some(value)
...
pub fn validate_f64_param_range(value: f64, min: f64, max: f64) -> Option<f64> {
    if value.is_nan() || value.is_infinite() {
        return None;
    }
...
pub fn validate_learning_rate(value: f64) -> Option<f64> {
    validate_f64_param_range(value, 0.0, 100.0)
}

pub fn validate_dropout_rate(value: f64) -> Option<f64> {
    validate_f64_param_range(value, 0.0, 1.0)
}

pub fn validate_l2_lambda(value: f64) -> Option<f64> {
    validate_f64_param_range(value, 0.0, 1000.0)
}

pub fn validate_activation_i32(val: i32) -> Option<TActivationType> {
    match val {
        0 => Some(TActivationType::AtSigmoid),
        1 => Some(TActivationType::AtTanh),
        2 => Some(TActivationType::AtReLU),
        3 => Some(TActivationType::AtSoftmax),
        _ => None,
    }
}

pub fn validate_optimizer_i32(val: i32) -> Option<TOptimizerType> {
    match val {
        0 => Some(TOptimizerType::OtSGD),
        1 => Some(TOptimizerType::OtAdam),
        2 => Some(TOptimizerType::OtRMSProp),
        _ => None,
    }
}
