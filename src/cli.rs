...
pub fn run() {
    ...
    let mut gpu_backend = String::from("auto");
    ...
    for i in 2..args.len() {
        ...
        if let Some(eq) = arg.find('=') {
            ...
            match key {
                ...
                "--batch" => _batch_size = value.parse().unwrap_or(1),
    ...
    match command {
        TCommand::CmdCreate => {
            ...
            let backend = match gpu_backend.as_str() {
                "auto" => select_best_backend(),
                other => TGPUBackend::from_str(other),
            };

            let mut mlp = match TMultiLayerPerceptronCUDA::new_with_backend(
                input_size,
                &hidden_sizes,
                output_size,
                hidden_act,
            ...
            println!("  GPU Backend: {}", mlp.get_backend().to_str());
        }
        TCommand::CmdTrain => {
        ...
        TCommand::CmdInfo => {
            ...
            println!("MLP Model Information (CUDA/OpenCL/Rust)");
            println!("  GPU Backend: {}", mlp.get_backend().to_str());
            println!("=================================");
